...
```{r}
mw.time <- wilcox.test(FBV$TIME.VISIT, SCNP$TIME.VISIT, paired = FALSE)
mw.time
mw.meantime <- wilcox.test(FBV$Mean_TIMEVISIT, SCNP$Mean_TIMEVISIT, paired = FALSE)
mw.meantime
```
...
where $\mu_{U} = \frac{n_{1}n_{2}}{2}$ and $\sigma_{U} = \sqrt{\frac{n_{1}n_{2}(n_{1}+n_{2}+1)}{12}}$
...
# now we should be able to plot this pretty easily
ggplot(data=SCNP_males, aes(x=ID, y=NUMBER.OF.PROBE.TOOLS)) +
  geom_bar(stat="identity", fill="red", width = 0.5) + labs(y = "no. probe tools") + scale_y_continuous(breaks = seq(0, 100, by = 10), expand=expansion(mult=c(0,0.01)))
```

this is kind of insane, but I don't like that they're in a different order. so i'm reordering the individuals in the dataset.
```{r}
...
ggplot(data=SCNP_males, aes(x=ID, y=PROBE.SUCCESS)) +
  geom_bar(stat="identity", fill="navy", width = 0.5) + labs(y = "proportion of successful probing") + scale_y_continuous(breaks = seq(0, 1.0, by = 0.1), expand = expansion(mult = c(0,0.05))) 
# i know the intervals could be more accurate, but frankly, i have no idea why they separated 0, 0.3, 0.5, 0.8, and 1.0 in equal intervals visually.
```